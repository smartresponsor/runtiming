...
$config = Join-Path $root "config\runtime\rr\rr.yaml"

Write-Host "Starting rr with config: $config"
$proc = Start-Process -FilePath "rr" -ArgumentList @("serve", "-c", $config) -PassThru -NoNewWindow
